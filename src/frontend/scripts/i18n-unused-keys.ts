...
class I18nUnusedKeysCleaner {
  ...
  async cleanupUnusedKeys(unusedKeys: UnusedKey[]): Promise<void> {
    ...
    // 按文件分组
    const byFile = unusedKeys.reduce((acc, key) => {
      if (!acc[key.file]) {
        acc[key.file] = [];
      }
   ...
   * 清理文件中的未使用键
   */
  private async cleanupFile(filePath: string, keys: UnusedKey[]): Promise<void> {
    try {
      const content = fs.readFileSync(filePath, 'utf-8');
  ...
  generateReport(unusedKeys: UnusedKey[]): string {
    ...
    // 按命名空间分组
    const byNamespace = unusedKeys.reduce((acc, key) => {
   ...
   * 保存报告到文件
   */
  async saveReport(unusedKeys: UnusedKey[], outputFile?: string): Promise<void> {
